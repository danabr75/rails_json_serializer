  ...
  def self.included klass
    ...
    klass.class_eval do
      ...
      if self.const_defined?("#{klass.name}Serializer")
        ...
        puts "KEYS2: #{serializer_query_names}"
        # SERIALIZER_QUERY_KEYS_CACHE = serializer_klass.public_instance_methods
        ...
        # CLASS SERIALIZER
        if !klass.respond_to?(:serializer)
        ...
        if !klass.respond_to?(:generate_includes_from_json_query)
        ...
        serializer_query_names.each do |query_name|
          ...
          if serializer_name == 'serializer'
            # No longer necessary to add here.
